...
def flash_firmware(port, firmware_bytes, progress_cb=None, log_cb=None):
    """Flash firmware_bytes to the ESP32-S3 at FLASH_OFFSET."""
    try:
        import esptool
    except ImportError:
        raise RuntimeError(
            "esptool no esta instalado.\n"
    ...
    try:
        ...
        if error:
            ...
            msg = lines[-1].strip() if lines else "Error desconocido de esptool"
...
class LemonFlasherApp:
    def __init__(self, root):
        ...
        style.configure("TCombobox",
                         fieldbackground=DARK, background=DARK,
                         foreground=GREENT, arrowcolor=GREENT,
                         bordercolor=MOON, selectbackground=NEBULA,
    ...
    def _build_ui(self):
        ...
        try:
            ...
            self._logo_img = tk.PhotoImage(data=logo_data)
            # Scale to ~200px wide
            scale = max(1, self._logo_img.width() // 200)
            self._logo_img = self._logo_img.subsample(scale, scale)
            tk.Label(main, image=self._logo_img, bg=BLACK).pack(pady=(4, 0))
        except Exception:
            tk.Label(main, text="LEMON", font=("Consolas", 20, "bold"),
                     bg=BLACK, fg=GREENT).pack(pady=(4, 0))

        tk.Label(main, text="Display Flasher v1.0",
                 font=MONO_SM, bg=BLACK, fg=NEBULA).pack()
        tk.Label(main, text="lemon team // 2026",
                 font=MONO_XS, bg=BLACK, fg=MOON).pack()

        self._sep(main)

        # ── USB Port ──
        tk.Label(main, text="[ USB PORT ]",
                 font=MONO_LG, bg=BLACK, fg=NEBULA).pack(anchor="w")

        port_row = tk.Frame(main, bg=BLACK)
        port_row.pack(fill="x", pady=(4, 0))

        tk.Label(port_row, text="Puerto:", font=MONO_SM,
                 bg=BLACK, fg=MOON).pack(side="left")

        self.port_combo = ttk.Combobox(port_row, textvariable=self.selected_port,
                                        state="readonly", width=12,
                                        style="TCombobox", font=MONO_SM)
        self.port_combo.pack(side="left", padx=(8, 8))

        tk.Button(port_row, text="SCAN", command=self._refresh_ports,
                  font=MONO_XS, bg=DARK, fg=NEBULA,
                  activebackground="#1A1A1A", activeforeground=GREENT,
                  bd=1, relief="ridge", cursor="hand2",
                  padx=8).pack(side="left")

        self.port_status = tk.Label(main, text="...",
                                     font=MONO_XS, bg=BLACK, fg=MOON)
        self.port_status.pack(anchor="w", pady=(2, 0))

        self._sep(main)

        # ── Firmware ──
        tk.Label(main, text="[ FIRMWARE ]",
                 font=MONO_LG, bg=BLACK, fg=NEBULA).pack(anchor="w")

        ver_row = tk.Frame(main, bg=BLACK)
        ver_row.pack(fill="x", pady=(4, 0))

        self.version_label = tk.Label(ver_row, text="Consultando...",
                                       font=MONO_SM, bg=BLACK, fg=MOON)
        self.version_label.pack(side="left")

        self.btn_check = tk.Button(ver_row, text="CHECK", command=self._check_version,
                  font=MONO_XS, bg=DARK, fg=NEBULA,
                  activebackground="#1A1A1A", activeforeground=GREENT,
                  bd=1, relief="ridge", cursor="hand2",
                  padx=8)
        ...
        self.progress_label = tk.Label(main,
                                        text=self._render_progress(0),
                                        font=MONO, bg=BLACK, fg=GREENT)
        self.progress_label.pack(fill="x")

        self.status_label = tk.Label(main, text="Listo",
                                      font=MONO_XS, bg=BLACK, fg=MOON)
        ...
        self.btn_flash = tk.Button(
            ...
            bg=GREENT, fg=BLACK,
            activebackground=EVERGREENT, activeforeground=BLACK,
        ...
        # ── Log ──
        tk.Label(main, text="[ LOG ]", font=MONO_XS, bg=BLACK,
                 fg=MOON).pack(anchor="w")

        log_frame = tk.Frame(main, bg=DARKER, bd=1, relief="sunken")
        log_frame.pack(fill="both", expand=True, pady=(4, 0))

        self.log_text = tk.Text(log_frame, height=8, bg=DARKER, fg=GREENT,
                                 font=MONO_XS, bd=0, wrap="word",
                                 insertbackground=GREENT, padx=8, pady=6)
    ...
    def _refresh_ports(self):
        ...
        if ports:
            ...
            self.port_status.config(
                text=f"{len(ports)} dispositivo(s) USB", fg=GREENT)
    ...
    def _on_version_found(self, ver, url, md5):
        ...
        if not url:
            self.version_label.config(
                text=f"v{ver} - sin .bin en release", fg=SOLAR)
    ...
    def _start_flash(self):
        ...
        if not port:
            messagebox.showwarning("Puerto",
                                    "Selecciona un puerto USB primero.")
            return
        if not self.asset_url:
            messagebox.showwarning("Firmware",
                                    "Primero consulta la version disponible.")
            return
        ...
        def _do():
            try:
                ...
                flash_firmware(port, fw, log_cb=log_cb)
    ...
    def _on_flash_success(self):
        ...
        self._log("El dispositivo se reiniciara automaticamente", "ok")
        messagebox.showinfo(
            "Listo",
            f"Firmware v{self.latest_version} instalado.\n"
            "El dispositivo se reiniciara solo.")
