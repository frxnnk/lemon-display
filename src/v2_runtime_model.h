...
constexpr V2Scene v2NextScene(V2Scene scene, TouchGesture gesture,
    ...
    return scene == V2_HOME && gesture == TOUCH_TAP &&
               x >= 384 && x <= 456 && y >= 360 && y <= 438 ? V2_SETTINGS
         : scene == V2_HOME &&
               (gesture == TOUCH_SWIPE_LEFT ||
                (gesture == TOUCH_TAP && y >= 352)) ? V2_MARKET_TAPE
         : scene == V2_MARKET_TAPE && gesture == TOUCH_TAP &&
               y >= 132 && y < 402 ? V2_CONTEXT
...
static_assert(v2NextScene(V2_HOME, TOUCH_TAP, 240, 400) == V2_MARKET_TAPE,
              "Home tape CTA must open Market Tape");
static_assert(v2NextScene(V2_HOME, TOUCH_TAP, 419, 399) == V2_SETTINGS,
              "The Home settings icon must open Settings");
static_assert(v2NextScene(V2_HOME, TOUCH_LONG_PRESS, 419, 399) == V2_HOME,
