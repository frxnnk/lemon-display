...
export function createWorkbench3D(container, screenCanvas, options = {}) {
  ...
  addLight(scene, 0x00df1a, 0.48, -70, 30, -50);
  ...
  loadModels(group, texture, state)
    .then(() => {
      ...
      setCameraView(options.initialView || "front");
  ...
  function setCameraView(view = "front") {
    ...
    const specs = {
      ...
      overview: { normal: 285, right: 10, up: 12, targetUp: -22 },
      screen: { normal: 68, right: 0, up: 4 },
      side: { normal: 72, right: 136, up: 14 },
    };
    const narrow = camera.aspect < 0.72;
    const base = specs[view] || specs.front;
    const target = state.screenWorld.clone().addScaledVector(basis.up, base.targetUp || 0);
    const spec = {
  ...
  function resize() {
    ...
    if (state.ready && !state.cameraTween) {
      if (options.initialView === "overview") setCameraView("overview");
      else setAssemblyProgress(state.progress);
    }
  }
...
async function loadModels(group, texture, state) {
  ...
  const green = new THREE.MeshPhysicalMaterial({ color: 0x00df1a, roughness: 0.48, metalness: 0.06, emissive: 0x00df1a, emissiveIntensity: 0.28 });
...
function makeHalo() {
  ...
  gradient.addColorStop(0, "rgba(0,223,26,0.18)");
  gradient.addColorStop(0.48, "rgba(0,223,26,0.05)");
  gradient.addColorStop(1, "rgba(0,223,26,0)");
